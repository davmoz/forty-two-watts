...
function driver_poll()
    ...
    -- Canonical keys: dc_W is the emitted PV power (negative = generating,
    -- measured at the inverter's AC output); the raw positive DC-side
    -- power stays on the legacy dc_w key as a diagnostic. Per-tracker
    -- pv_mppt{n}_v/a TS series come from the host's mppts[] fan-out.
    host.emit("pv", {
        dc_W                = -ac_w,
        dc_w                = dc_w,
        mppts               = {
